...
class ReplaceManager
{
    ...
    protected function replaceLinks($content)
    {
        if ($this->helper->isActive()) {
            ...
            $content = $this->removeClassContent($content);
            ...
            $content = $this->returnClassContent($content);
    ...
    private function removeClassContent(string $content): string
    {
        $content = preg_replace_callback(
            '#(class=")(.*?)((\/\>)|("))#ims',
            [$this, 'replaceClass'],
    ...
    private function replaceClass(array $matches): string
    {
        $text = '';
        if (count($matches) >= 4) {
            $this->replacedClass[] = $matches[2];
            $text = $matches[1] . '{{CROSS_LINK_CLASS_' . (count($this->replacedClass) - 1) . '}}'
                . $matches[3];
        } elseif (isset($matches[0])) {
            $text = $matches[0];
        }
    ...
    private function returnClassContent(string $content): string
    {
        foreach ($this->replacedClass as $key => $class) {
            $content = str_replace('{{CROSS_LINK_CLASS_' . $key . '}}', $class, $content);
        }
